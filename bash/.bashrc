...
function parse_git_branch() {
    ...
    printf '%s%s ' "$b" "$(_git_dirty)"
}

# Single git call for the full prompt's " on branch" segment.
# Uses \001/\002 (not \[/\]) so color codes work inside $(...) in PS1.
function _prompt_git() {
    local b
    b=$(git symbolic-ref --short HEAD 2>/dev/null) || return
    printf ' on \001\033[1;33m\002%s%s\001\033[m\002 ' "$b" "$(_git_dirty)"
}

function _reboot_required() {
    case "$OSTYPE" in
        linux-gnu*)
...
function prompt () {
    ...
    case "$arg" in
        ...
        --full|-f)
            ...
            _ps1="$(_c $C_LIGHTBLUE '\u') at $(_c $C_CYAN '\h') in $(_c $C_LIGHTBLUE '\w')\$(_prompt_git)$(_c $C_GREEN '\$') "
            ;;
    esac
...
case "$OSTYPE" in
    ...
    *) os_name=$(uname) ;;  # Fallback for unknown platforms
esac

if [ -f ~/.bashrc_local_${os_name} ]; then
    source ~/.bashrc_local_${os_name}
fi
